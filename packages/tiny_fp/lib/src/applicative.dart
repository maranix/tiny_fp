...
/// Applicative interface for High-Kinded Types (HKT) with two different type paramters.
///
/// The `Applicative` interface is an extension of `Functor`, providing the ability
/// to apply a function wrapped inside a container to a value wrapped inside another container.
/// It allows you to work with values that are wrapped in a context (or container) and apply
...
/// Example:
/// ```dart
/// final hktFunc = Either("Wrapped Func", (int x) => x * 2); // A function wrapped in Box
/// final hktValue = Either("Wrapped value", 21); // A value wrapped in Box
/// final result = hktValue.ap(hktFunc);
/// assert(result == Either("Wrapped value", 42));     // The right [B] value inside `Either()` was doubled.
/// ```
abstract interface class Applicative2<C extends HKTMarker, A, B>
    extends Functor2<C, A, B> {
  /// Wraps a raw value of type [T] into the container replacing [B] and leaving [A] as it is.
  ///
  /// This method takes a raw value and wraps it inside the container (or context).
  ///
  /// - [value]: The raw value of type [T] to be wrapped in the container.
  /// - Returns: A new container of type [C] that holds the value.
  ///
  /// Example:
  /// ```dart
  /// final result = Either("Some", 4).pure(42);
  /// assert(result == Either("Some", 42));  // The value is wrapped inside `Either()` and `Some` was left intact.
  /// ```
  HKT2<C, A, T> pure<T>(T value);

  /// Applies a function inside container to second [B] value inside another container.
  ///
  /// This method allows you to apply a function that is inside one container
  /// to a value inside another container. The result is a new container that holds
  /// the result of the function application.
  ///
  /// - [hktFunc]: A container that holds a function of type [T Function(B)].
  /// - Returns: A new container of type [C] that holds the result of applying the function to the value.
  ///
  /// Example:
  /// ```dart
  ...
  /// the implementation should handle it appropriately.
  ///
  HKT2<C, A, T> ap<T>(HKT2<C, A, T Function(B)> hktFunc);
}

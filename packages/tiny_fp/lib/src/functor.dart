...
/// Functor interface for High-Kinded Types with two different type definitions.
///
/// The `Functor` interface represents a type that can be mapped over. It provides
/// a way to apply a function to the value inside a container (HKT) and return a new
/// container with the transformed value.
...
/// Example:
/// ```dart
/// final either = Either("Two", 2);
/// final result = either.map((value) => value * 2);
/// assert(result == Box(84)); // The value inside the box was doubled.
/// ```
abstract interface class Functor2<C extends HKTMarker, A, B> {
  /// Maps a function `f` over the value inside the container.
  ///
  /// This method takes a function that transforms the value inside the container
  /// and applies it, returning a new container with the transformed value.
  ///
  /// - [f]: A function that takes a value of type [B] and returns a value of type [T] without modifiying the [A] value.
  /// - Returns: A new container of type [C] with the transformed value [T] of type [R] and [A] intact.
  ///
  /// Example:
  /// ```dart
  /// final either = Either("Fourty Two", 42);
  /// final result = maybe.either((value) => value * 2);
  /// assert(result == Either("Foutry Two", 84)); // The value was doubled inside the container and the [A] was kept as it is.
  /// ```
  HKT2<C, A, T> map<T>(T Function(B) f);
}
